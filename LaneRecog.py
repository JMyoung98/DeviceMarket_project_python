...
def draw_lines(frame, lines, color=[0, 0, 255], thickness=2): # 선 그리기
    cv2.line(frame, (lines[0], lines[1]), (lines[2], lines[3]), color, thickness=0.3)
...
def top_view(frame, width, height):
    ...
    '''
    left_bottom = [width*0.25,height*0.75]
    right_bottom = [width*0.75,height*0.75]
    left_top = [(width*0.45)*0.25,height*0.25]
